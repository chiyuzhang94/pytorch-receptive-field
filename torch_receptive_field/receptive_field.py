...
def check_same(stride):
    if isinstance(stride, (list, tuple)):
            assert (len(stride) == 2 and stride[0] == stride[1]) or (len(stride) == 3 and stride[0] == stride[1] and stride[1] == stride[2]) or (len(stride) == 1)
            stride = stride[0]
    return stride
...
def receptive_field(model, input_size, batch_size=-1, device="cuda"):
    ...
    def register_hook(module):
        ...
        def hook(module, input, output):
            ...
            if not receptive_field["0"]["conv_stage"]:
                ...
                if class_name == "Conv2d" or class_name == "MaxPool2d" or class_name == "AvgPool2d" or class_name == "Conv3d" or class_name == "MaxPool3d" or class_name == "Conv1d" or class_name == "MaxPool1d" or class_name == "AvgPool1d":
                    kernel_size = module.kernel_size
                    stride = module.stride
                    padding = module.padding

                    if class_name == "AvgPool2d" or class_name == "AvgPool3d" or class_name == "AvgPool1d":
                ...
                elif class_name == "BatchNorm2d" or class_name == "ReLU" or class_name == "Bottleneck" or class_name == "BatchNorm3d" or class_name == "BatchNorm1d":
                    receptive_field[m_key]["j"] = p_j
                    receptive_field[m_key]["r"] = p_r
                    receptive_field[m_key]["start"] = p_start
                elif class_name == "ConvTranspose2d" or class_name == "ConvTranspose3d" or class_name == "ConvTranspose1d":
    ...
    for layer in receptive_field:
        ...
        assert len(receptive_field[layer]["output_shape"]) == 4 or len(receptive_field[layer]["output_shape"]) == 5 or len(receptive_field[layer]["output_shape"]) == 3
...
def receptive_field_for_unit(receptive_field_dict, layer, unit_position):
    ...
    if layer in receptive_field_dict:
        ...
        assert len(unit_position) == 2 or len(unit_position) == 3 or len(unit_position) == 1
        ...
        elif len(unit_position) == 1:
            if len(input_shape) == 1:
                limit = input_shape
            else:  # input shape is (channel, W)
